...
/**
 * Activity Creator Engine
 * Handles the direct interaction with Moodle's core APIs to instantiate courses,
 * pages, quizzes, feedbacks, and certificates.
 */
class ActivityCreator {
    
    // Safety Mechanism: Database Transaction Cleanser
    // Moodle crashes if a previous API call left an uncommitted transaction open.
    // This forcibly clears the transaction stack before initiating new heavy operations.
    ...
    // Ensures complex nested JSON structures from the AI are safely converted to strings
    // to prevent database insertion errors in text fields.
    ...
    // Page Generator
    // Creates a Moodle "Page" module using the HTML content generated by the AI.
    // Automatically enforces access restrictions based on the $prerequisite_ids array.
    public static function create_page($course_id, $activity, $section = 1, $prerequisite_ids = []) {
        ...
        // Access Restrictions Injection
        // Locks the page until previous pages (or the quiz, if applicable) are completed.
    ...
    // Quiz Engine Builder
    // Creates a robust assessment environment. It registers the Quiz module, configures
    // the grading logic (pass grade, multiple attempts), and dynamically pulls questions
    // from the generated Question Banks to populate the exam.
    public static function create_quiz($course_id, $activity, $json_data = null, $section = 1, $prerequisite_ids = []) {
        ...
        // Enforces that the student must achieve a passing grade (15/20) to complete the quiz.
        ...
        // Compatibility attributes required for Moodle 5.x API
        ...
        // Access Restrictions Injection
        // Locks the quiz until all preceding HTML pages are marked as viewed.
        if (!empty($prerequisite_ids)) {
            $conditions = [];
            $showc = [];
            foreach ($prerequisite_ids as $cmid) {
    ...
    // Evaluation / Feedback Generator
    // Instantiates a Moodle Feedback module to collect user satisfaction metrics.
    // It loads a predefined XML structure containing standardized questions and inserts it into the database.
    ...
    // Certificate Engine
    // Locates the global Master Template for certificates and safely clones it into the context of the newly created course.
    // This deep-copy prevents database corruption issues ("Multiple records found") when multiple courses try to edit the same template.
    public static function create_certificate($course_id, $template_name = 'LMS-AI_Certificate') {
        ...
        try {
            ...
            // Private Template Cloning
            // We create a dedicated copy of the template for this specific course instance.
