<?php
/**
 * External API for creating page activities - v5.1 (TESTE AXIOMA -1)
 * Lógica alterada para testar a hipótese de cm: -1 ser a atividade anterior.
 */
...
class create_page extends external_api {
    ...
    public static function execute_parameters() {
        return new external_function_parameters([
            ...
            // ALTERAÇÃO 1: Descrição do parâmetro atualizada.
            'completiontype' => new external_value(PARAM_INT, 'Tipo de conclusão (0=nenhum, 1=automático por visualização)', VALUE_DEFAULT, 0),
            'sequential' => new external_value(PARAM_BOOL, 'Se true, adiciona restrição de atividade anterior', VALUE_DEFAULT, false),
            'options' => new external_single_structure([
                'intro' => new external_value(PARAM_RAW, 'Introduction', VALUE_DEFAULT, ''),
                'introformat' => new external_value(PARAM_INT, 'Format', VALUE_DEFAULT, FORMAT_HTML),
                'visible' => new external_value(PARAM_BOOL, 'Visible', VALUE_DEFAULT, true),
                'groupmode' => new external_value(PARAM_INT, 'Group mode', VALUE_DEFAULT, 0),
                'groupingid' => new external_value(PARAM_INT, 'Grouping ID', VALUE_DEFAULT, 0),
                'availability' => new external_value(PARAM_RAW, 'Availability', VALUE_DEFAULT, null),
                'completion' => new external_single_structure([
                    'completionview' => new external_value(PARAM_BOOL, 'View completion', VALUE_DEFAULT, false),
                    'completionexpected' => new external_value(PARAM_INT, 'Expected date', VALUE_DEFAULT, 0),
                ], 'Completion', VALUE_OPTIONAL),
            ], 'Options', VALUE_OPTIONAL)
        ]);
    }
    
    public static function execute($courseid, $sectionnum, $name, $content, $completiontype = 0, $sequential = false, $options = []) {
        global $CFG, $DB;
        ...
        try {
            ...
            // ALTERAÇÃO 2: Lógica de conclusão modificada para corresponder ao requisito.
            if (!empty($params['completiontype']) && $params['completiontype'] == 1) {
                ...
                // Mantém o comportamento original para outros casos (nenhum ou manual, se aplicável).
                $moduleinfo->completion = $params['completiontype'];
            }

            // A sua lógica original para o teste do axioma -1 permanece intacta.
            if ($params['sequential']) {
                $availability_rules = ['op' => '&', 'c' => [['type' => 'completion', 'cm' => -1, 'e' => 1]], 'showc' => [true]];
                $moduleinfo->availability = json_encode($availability_rules);
            }
