...
/**
 * Create Quiz - API
 * VERSÃO 37.1 - VALORES REAIS DESCOBERTOS NA BD + REVIEWMAXMARKS
 *
 * SOLUÇÃO DEFINITIVA v37.1:
 * - 🎯 VALORES REAIS: Baseados nos testes reais da base de dados
 * - 🧮 DESCOBERTAS: 69904, 4096, 4352, 16, 4112, 4352 (valores que funcionam)
 * - ✅ TESTADO: Quiz ID 446 com configuração funcionante
 * - 📊 BASEADO EM: Valores reais do Moodle após correções automáticas
 * - 🆕 ADICIONADO: reviewmaxmarks = 4352 (Maximum marks)
 *
 * @package    local_wsmanageactivities
 * @copyright  2025 Your Name
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @version    37.1
 * @date       25 de Janeiro de 2025, 19:45
 */
...
class create_quiz extends external_api {
    ...
    /**
     * ✅ CONSTANTES v37.1: Valores Reais da Base de Dados
    ...
    // ✅ v37.1: Valores reais descobertos na BD do Quiz ID 446
    ...
    const REVIEW_MAXMARKS_VALUE = 4352;           // Maximum marks (Immediately + Later) - valor real da BD
    ...
    public static function execute_parameters() {
        return new external_function_parameters([
            ...
            // ===== REVIEW OPTIONS v37.1: VALORES REAIS DA BD =====
            ...
            'reviewmaxmarks' => new external_value(PARAM_INT, 'Review maximum marks settings', VALUE_DEFAULT, self::REVIEW_MAXMARKS_VALUE),
            
            // ===== DEBUG E VALIDAÇÃO v37.1 =====
    ...
    // ✅ v37.1: Parâmetros com valores reais da BD
    public static function execute($courseid, $sectionnum, $name, $intro = '', $grade = 10.0, $attempts = 0, $timelimit = 0, $timeopen = 0, $timeclose = 0, $visible = true, $completiontype = 1, $sequential = true, $prerequisitecmid = 0, $gradepass = null, $reviewattempt = self::REVIEW_ATTEMPT_VALUE, $reviewcorrectness = self::REVIEW_CORRECTNESS_VALUE, $reviewmarks = self::REVIEW_MARKS_VALUE, $reviewspecificfeedback = self::REVIEW_SPECIFIC_FB_VALUE, $reviewgeneralfeedback = self::REVIEW_GENERAL_FB_VALUE, $reviewrightanswer = self::REVIEW_RIGHT_ANSWER_VALUE, $reviewoverallfeedback = self::REVIEW_OVERALL_FB_VALUE, $reviewmaxmarks = self::REVIEW_MAXMARKS_VALUE, $validate_review_bits = true, $debug_bit_analysis = true) {
        global $DB, $CFG;

        // === DEBUG LOG INICIAL v37.1 ===
        debugging("v37.1 DEBUG: === REVIEW OPTIONS COM VALORES REAIS DA BD ===", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewattempt = " . self::REVIEW_ATTEMPT_VALUE . " (the attempt todos os períodos)", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewcorrectness = " . self::REVIEW_CORRECTNESS_VALUE . " (whether correct só immediately)", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewmarks = " . self::REVIEW_MARKS_VALUE . " (marks immediately + later)", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewrightanswer = " . self::REVIEW_RIGHT_ANSWER_VALUE . " (right answer after closed)", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewoverallfeedback = " . self::REVIEW_OVERALL_FB_VALUE . " (overall feedback immediately + after closed)", DEBUG_DEVELOPER);
        debugging("v37.1 DEBUG: reviewmaxmarks = " . self::REVIEW_MAXMARKS_VALUE . " (maximum marks)", DEBUG_DEVELOPER);

        $grade = (float)$grade;
        $calculated_gradepass = self::calculate_gradepass_correctly($grade, $gradepass);
        
        // === v37.1: ANÁLISE DE BITS SE SOLICITADA ===
        if ($debug_bit_analysis) {
            ...
            debugging("v37.1 DEBUG: Bit analysis completed", DEBUG_DEVELOPER);
        }
        ...
        $params = self::validate_parameters(self::execute_parameters(), 
            compact('courseid', 'sectionnum', 'name', 'intro', 'grade', 'attempts', 
                   ...
                   'reviewmaxmarks', 'validate_review_bits', 'debug_bit_analysis'));
        ...
        try {
            ...
            try {
                // ===== CRIAR QUIZ COM REVIEW OPTIONS REAIS v37.1 =====
                $quiz = new stdClass();
                ...
                // ✅ v37.1: VALORES REVIEW REAIS DA BD
                ...
                $quiz->reviewmaxmarks = (int)$params['reviewmaxmarks'];
                
                debugging("v37.1 DEBUG: Quiz object review values (REAIS DA BD):", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewattempt: {$quiz->reviewattempt} (69904 - the attempt todos)", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewcorrectness: {$quiz->reviewcorrectness} (4096 - whether correct immediately)", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewmarks: {$quiz->reviewmarks} (4352 - marks immediately + later)", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewrightanswer: {$quiz->reviewrightanswer} (16 - right answer after closed)", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewoverallfeedback: {$quiz->reviewoverallfeedback} (4112 - overall immediately + after)", DEBUG_DEVELOPER);
                debugging("v37.1 DEBUG: - reviewmaxmarks: {$quiz->reviewmaxmarks} (4352 - maximum marks)", DEBUG_DEVELOPER);
                ...
                debugging("v37.1 DEBUG: Quiz inserido com ID: {$quiz->id}", DEBUG_DEVELOPER);

                // === v37.1: VERIFICAÇÃO PÓS-INSERÇÃO ===
                $verification_result = self::verify_review_options_v37($quiz->id, [
                    ...
                    'reviewmaxmarks' => $params['reviewmaxmarks'],
                ]);
                ...
                debugging("v37.1 DEBUG: Quiz criado com sucesso - ID: {$quiz->id}, CM: {$cm->id}", DEBUG_DEVELOPER);
                ...
                return [
                    ...
                    // ===== v37.1: REVIEW OPTIONS COM VALORES REAIS =====
                    'review_options_v37' => json_encode([
                        'configuration_type' => 'real_database_values',
                        'values_applied' => [
                            'reviewattempt' => self::REVIEW_ATTEMPT_VALUE,
                            'reviewcorrectness' => self::REVIEW_CORRECTNESS_VALUE,
                            'reviewmarks' => self::REVIEW_MARKS_VALUE,
                            'reviewrightanswer' => self::REVIEW_RIGHT_ANSWER_VALUE,
                            'reviewoverallfeedback' => self::REVIEW_OVERALL_FB_VALUE,
                            'reviewmaxmarks' => self::REVIEW_MAXMARKS_VALUE,
                        ],
                    ...
                    'debug_info' => [
                        ...
                        'version' => '37.1',
                        'review_configuration' => 'Real database values from Quiz ID 446 + reviewmaxmarks',
            ...
            debugging("v37.1 ERROR: " . $e->getMessage(), DEBUG_DEVELOPER);
            return [
                'id' => 0,
                'instance' => 0,
                'name' => $params['name'],
    ...
    /**
     * ✅ FUNÇÃO v37.1: Analisar bits das review options reais
     */
    private static function analyze_review_bits($review_values) {
        ...
        foreach ($review_values as $field => $value) {
            ...
            debugging("v37.1 BIT ANALYSIS $field: $value = $binary, periods: " . implode('+', $periods_active), DEBUG_DEVELOPER);
        }
    ...
    /**
     * ✅ FUNÇÃO v37.1: Verificar se coincide com especificação
     */
    private static function verify_specification_match($field, $periods_active) {
        $expected_periods = [
            ...
            'reviewmaxmarks' => ['IMMEDIATELY_AFTER', 'LATER_WHILE_OPEN'],
        ];
    ...
    /**
     * ✅ FUNÇÃO v37.1: Verificar review options após inserção
     */
    private static function verify_review_options_v37($quiz_id, $expected_values) {
        ...
        $actual_quiz = $DB->get_record('quiz', ['id' => $quiz_id], 
            'reviewattempt, reviewcorrectness, reviewmarks, reviewspecificfeedback, reviewgeneralfeedback, reviewrightanswer, reviewoverallfeedback, reviewmaxmarks', 
            MUST_EXIST);
        ...
        foreach ($expected_values as $field => $expected_value) {
            ...
            debugging("v37.1 VERIFY $field: Expected=$expected_value, Actual=$actual_value, Correct=" . ($is_correct ? 'YES' : 'NO'), DEBUG_DEVELOPER);
        }
    ...
    /**
     * ✅ FUNÇÃO v37.1: Obter períodos ativos de um valor
     */
    ...
    /**
     * FUNÇÕES MANTIDAS DAS VERSÕES ANTERIORES (v29.0-v37.0)
     */
    private static function calculate_gradepass_correctly($grade, $gradepass) {
        debugging("v37.1 DEBUG: calculate_gradepass_correctly - grade=$grade, gradepass=" . ($gradepass === null ? 'NULL' : $gradepass), DEBUG_DEVELOPER);
        
        if ($gradepass !== null) {
            $gradepass = (float)$gradepass;
            
            if ($gradepass < 0) {
                debugging("v37.1 WARNING: gradepass negativo ($gradepass) alterado para 0", DEBUG_DEVELOPER);
                return 0.0;
            }
            
            if ($gradepass > $grade) {
                debugging("v37.1 WARNING: gradepass ($gradepass) > grade ($grade), ajustado para grade", DEBUG_DEVELOPER);
                return (float)$grade;
            }
            
            debugging("v37.1 DEBUG: Usando gradepass fornecido: $gradepass", DEBUG_DEVELOPER);
            return $gradepass;
        }
        
        $calculated = round($grade * 0.8, 2);
        debugging("v37.1 DEBUG: Calculado 80% automaticamente: $calculated (de $grade)", DEBUG_DEVELOPER);
        return $calculated;
    }
    ...
    private static function create_or_update_grade_item_v29($quiz, $gradepass) {
        ...
        debugging("v37.1 DEBUG: Criando grade_item com gradepass=$gradepass", DEBUG_DEVELOPER);
        ...
        if ($existing_item) {
            ...
            debugging("v37.1 DEBUG: Grade_item atualizado ID={$existing_item->id} com gradepass=$gradepass", DEBUG_DEVELOPER);
            ...
            debugging("v37.1 DEBUG: Grade_item criado ID=$grade_item_id com gradepass=$gradepass", DEBUG_DEVELOPER);
    ...
    public static function execute_returns() {
        return new external_single_structure([
            ...
            'review_options_v37' => new external_value(PARAM_RAW, 'Review options v37.1 information as JSON', VALUE_OPTIONAL),
